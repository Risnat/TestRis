...
class ControlRechercherTrajet extends CI_Controller 
   ...
   private $place_id;
   private $vers_lycee;
   private $trajet_trouve;
   private $trajet_demande;
   private $trajet_choisi;
   private $formulaire1_reussi;
   private $formulaire2_reussi;
...
{ if ($this->trajet_demande==false && $this->formulaire1_reussi==false)
  ...
  if($this->trajet_demande==true && $this->formulaire1_reussi==false)
    ...
    if($this->trajet_demande==true && $this->formulaire1_reussi==true && $this->trajet_trouve==false && $this->formulaire2_reussi==false)
 ...
 if( $this->trajet_trouve==true && $this->formulaire2_reussi==false)
      {   //  Le formulaire 2 est invalide ou vide 
        $this->load->library('layout');
        $this->layout->set_theme('default');
        $this->layout->ajouter_css('jumbotron');   
        $data['results']=$this->trajets_trouves;
        echo "data ici5";print_r($data);
        $this->layout->views('themes/barreNav')
               ->views('vueConfirmeTrajetRecherche',$data)
                ->view('themes/footer');
      }
   
   if( $this->trajet_trouve==true && $this->formulaire2_reussi==true)         
      { //maj du trajet en retirant un passager et préparation 
         $num_trajet_choisi=$this->input->post('choixTrajet')-1;
         echo "num". $num_trajet_choisi;
         $ligne_trajet_choisi=$this->trajets_trouves[$num_trajet_choisi];
         $this->load->model('realise_model');
         $this->realise_model->maj_realise($this->session->id_covoit_connecte,$ligne_trajet_choisi[$num_trajet_choisi]->id_trajet,$ligne_trajet_choisi[$num_trajet_choisi]->id_conducteur);
         $trajet_choisi=true;
        $this->load->library('layout');
        $this->layout->set_theme('default');
        $this->layout->ajouter_css('jumbotron');  
